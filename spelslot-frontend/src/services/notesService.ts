...
export const notesService = {
  ...
  async load(sessionId: string, noteType: NoteType, noteId: string): Promise<Result<NoteDetail>> {
    const r = await api.get<{ note: NoteDetail }>(
      `/api/session-notes/${sessionId}/${noteType}/${noteId}`,
    )
    if (r.type === 'error') return r
  ...
  async create(sessionId: string, noteType: NoteType, name?: string): Promise<Result<NoteDetail>> {
    const r = await api.post<{ note: NoteDetail }>(
      `/api/session-notes/${sessionId}/${noteType}`,
      name ? { name } : {},
    )
    if (r.type === 'error') return r
    return { type: 'ok', data: r.data.note }
  ...
  async save(
    ...
    noteId: string,
    content: object,
  ): Promise<Result<NoteMeta>> {
    const r = await api.put<{ note: NoteMeta }>(
      `/api/session-notes/${sessionId}/${noteType}/${noteId}`,
      { content },
    )
    if (r.type === 'error') return r
