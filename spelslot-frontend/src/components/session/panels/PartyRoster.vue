    ...
    <template v-else-if="detail">
      ...
      <div class="roster__stats">
        <div class="roster__stat">
          ...
          <span class="roster__stat-label">Assigned</span>
        </div>
        <div class="roster__stat-divider" />
        <div class="roster__stat">
          <span class="roster__stat-val">{{ detail.maxPlayers }}</span>
          <span class="roster__stat-label">Max</span>
        </div>
        <div class="roster__stat-divider" />
        <div class="roster__stat">
          <span
          ...
          <span class="roster__stat-label">Free</span>
        </div>
        <template v-if="detail.waitlist.length">
          <div class="roster__stat-divider" />
          <div class="roster__stat">
            <span class="roster__stat-val roster__stat-val--wait">{{ detail.waitlist.length }}</span>
            <span class="roster__stat-label">Waiting</span>
          </div>
        </template>
      </div>
      ...
      <div v-if="detail.waitlist.length" class="roster__tabs">
        ...
        >
          <i class="pi pi-clock" /> Waiting list ({{ detail.waitlist.length }})
        </button>
      </div>
      ...
      <ul v-if="activeTab === 'party'" class="roster__list">
        ...
        <li
          v-for="(player, i) in detail.party"
          :key="player.id"
      ...
      <div class="roster__footer">
        ...
        <Tag v-if="detail.isStoryAdventure" value="Story" severity="warn" class="roster__story-tag" />
      </div>
...
.roster__empty-icon { font-size: 1.75rem; opacity: 0.3; }
.roster__empty p { margin: 0; font-size: 0.8rem; }
.roster__empty--error .roster__empty-icon { color: var(--ss-danger); opacity: 1; }
...
.roster__stat-val--zero { color: var(--ss-danger); }
.roster__stat-val--wait { color: var(--ss-warning, #f59e0b); }
...
.roster__tab:hover { color: var(--ss-text); }
.roster__tab--active { color: var(--ss-primary); border-bottom-color: var(--ss-primary); }
.roster__tab .pi { font-size: 0.65rem; }
...
.roster__player--waitlist { opacity: 0.7; }
...
.roster__player-tag { font-size: 0.6rem !important; }
...
.roster__footer-dm .pi { color: var(--ss-primary); font-size: 0.65rem; }
.roster__story-tag { font-size: 0.6rem !important; }
</style>
