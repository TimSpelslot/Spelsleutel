...
async function switchNote(noteId: string) {
  ...
  if (saveTimer.value) {
    ...
    if (activeId.value) {
      await notesService.save(props.sessionId, 'dm', activeId.value, editor.value!.getJSON())
    }
  }
...
async function addNote() {
  ...
  const r = await notesService.create(props.sessionId, 'dm')
  if (r.type === 'ok') {
...
<template>
  <div class="dm-notes">
    <!-- Tab bar -->
    <div class="dm-notes__tabs">
      <div class="dm-notes__tab-list">
        <button
          v-for="note in notes"
          :key="note._id"
          class="dm-notes__tab"
          :class="{ 'dm-notes__tab--active': note._id === activeId }"
...
.dm-notes__editor :deep(.ProseMirror) {
  outline: none;
  min-height: 100%;
  color: var(--ss-text);
  line-height: 1.7;
