...
function ok(msg: string) {
  console.log(`${GRN}✓${RESET} ${msg}`)
}
function warn(msg: string) {
  console.log(`${YEL}⚠${RESET} ${msg}`)
}
function fail(msg: string) {
  console.log(`${RED}✗${RESET} ${msg}`)
}
function info(msg: string) {
  console.log(`${CYN}ℹ${RESET} ${msg}`)
}
...
async function main() {
  ...
  if (!filePath) {
    console.error('Usage: tsx src/scripts/auditLk.ts <file.lk>')
    process.exit(1)
  }
  ...
  const dbEntries = await WorldEntry.find(
    {},
    'lkId name slug parentId permission isLocked tags aliases',
  ).lean()
  ...
  if (dbDocs.filter((d) => d.lkDocId).length === lkAllDocs.length) {
    ...
    warn(
      `Documents: ${lkAllDocs.length} in LK, ${dbDocs.filter((d) => d.lkDocId).length} matched in DB`,
    )
  }
  ...
  if (dbRels.length >= lkLinkTargets - 2) {
    ok(
      `Relations: ${dbRels.length} in DB vs ${lkLinkTargets} actual links in LK (${lkLinks.length} RESOURCE_LINK containers, most empty)`,
    )
  } else {
    warn(
      `Relations: ${dbRels.length} in DB vs ${lkLinkTargets} actual links in LK (${lkLinks.length} RESOURCE_LINK containers)`,
    )
  }
  ...
  if (missingEntries.length === 0) {
    ...
    missingEntries.slice(0, 20).forEach((r) => console.log(`  ${DIM}${r.id}${RESET}  "${r.name}"`))
    if (missingEntries.length > 20)
      console.log(`  ${DIM}… and ${missingEntries.length - 20} more${RESET}`)
  }
  ...
  if (missingDocs.length === 0) {
    ...
    if (missingDocs.length > 20)
      console.log(`  ${DIM}… and ${missingDocs.length - 20} more${RESET}`)
  }
  ...
  if (withParentInDB.length === withParentInLK.length) {
    ...
    if (missingParents.length > 0) {
      ...
      if (missingParents.length > 10)
        console.log(`  ${DIM}… and ${missingParents.length - 10} more${RESET}`)
    } else {
  ...
  for (const lkr of sampleLk) {
    ...
    const tagsOk =
      JSON.stringify((dbEntry.tags ?? []).sort()) === JSON.stringify((lkr.tags ?? []).sort())
    const aliasOk =
      JSON.stringify((dbEntry.aliases ?? []).sort()) === JSON.stringify((lkr.aliases ?? []).sort())

    console.log(`\n  "${lkr.name}" ${DIM}(lkId: ${lkr.id})${RESET}`)
    nameOk ? ok('  name matches') : fail(`  name mismatch: LK="${lkr.name}" DB="${dbEntry.name}"`)
    permOk
      ? ok(`  permission correct (${dbEntry.permission})`)
      : fail(
          `  permission: expected ${lkr.isHidden ? 'DM_ONLY' : 'PLAYERS'}, got ${dbEntry.permission}`,
        )
    tagsOk
      ? ok(`  tags match (${(lkr.tags ?? []).join(', ') || 'none'})`)
      : warn(`  tags: LK=[${lkr.tags?.join(', ')}] DB=[${dbEntry.tags?.join(', ')}]`)
    aliasOk
      ? ok(`  aliases match (${(lkr.aliases ?? []).join(', ') || 'none'})`)
      : warn(`  aliases: LK=[${lkr.aliases?.join(', ')}] DB=[${dbEntry.aliases?.join(', ')}]`)
  ...
  dbDocs.forEach((d) => {
    dbDocTypes[d.type] = (dbDocTypes[d.type] ?? 0) + 1
  })
  Object.entries(dbDocTypes).forEach(([type, count]) => info(`  ${type.padEnd(8)}: ${count}`))
  ...
  if (entriesNoDocs.length === 0) {
    ...
    entriesNoDocs.slice(0, 15).forEach((e) => console.log(`  ${DIM}${e.lkId}${RESET}  "${e.name}"`))
    if (entriesNoDocs.length > 15)
      console.log(`  ${DIM}… and ${entriesNoDocs.length - 15} more${RESET}`)
  }
